...
public class BitmapFactory {
    ...
    private CircularList<SpinnerItem> numericSpinnerItems = new CircularList<SpinnerItem>(new ArrayList<SpinnerItem>());;
    private CircularList<SpinnerItem> alphaLowerCaseSpinnerItems = new CircularList<SpinnerItem>(new ArrayList<SpinnerItem>());;
    private CircularList<SpinnerItem> alphaUpperCaseSpinnerItems = new CircularList<SpinnerItem>(new ArrayList<SpinnerItem>());;
    private CircularList<SpinnerItem> specialCharacterSpinnerItems = new CircularList<SpinnerItem>(new ArrayList<SpinnerItem>());;
    private CircularList<SpinnerItem> defaultSpinnerItem = new CircularList<SpinnerItem>(new ArrayList<SpinnerItem>());;
    ...
    private SpinnerItem createSpinnerItem(GVRContext gvrContext, String spinnerText, int position, int mode) {
        ...
        SpinnerItem textSpinner = new SpinnerItem(gvrContext, sceneObjectWidth, sceneObjectHeigth, bitmapWidth, bitmapHeigth, position, text);
    ...
    public void init(GVRContext gvrContext) {
        ...
        for (int i = 0; i < softKeyboard.length(); i++) {
            alphaLowerCaseSpinnerItems.add(createSpinnerItem(gvrContext, softKeyboard.getString(i), i, Keyboard.SOFT_KEYBOARD_LOWERCASE));

        }

        for (int i = 0; i < softKeyboard.length(); i++) {
            alphaUpperCaseSpinnerItems
                    .add(createSpinnerItem(gvrContext, softKeyboard.getString(i).toUpperCase(), i, Keyboard.SOFT_KEYBOARD_UPPERCASE));
        }

        for (int i = 0; i < numericKeyboard.length(); i++) {
            numericSpinnerItems.add(createSpinnerItem(gvrContext, numericKeyboard.getString(i).toUpperCase(), i, Keyboard.NUMERIC_KEYBOARD));
        }

        for (int i = 0; i < SPINNER_ITEM_SIZE; i++) {
            defaultSpinnerItem.add(createSpinnerItem(gvrContext, numericKeyboard.getString(i).toUpperCase(), i, Keyboard.NUMERIC_KEYBOARD));
        }
