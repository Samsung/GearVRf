...
public class SpinnerItemFactory {
    ...
    private Bitmap createSpinnerItem(String spinnerText) {
        ...
        return GVRTextBitmapFactory.create(gvrContext.getContext(), bitmapWidth, bitmapHeigth, text, 0);
    }

    public void init() {

        for (int i = 0; i < CharList.getInstance(gvrContext).getList(Keyboard.SOFT_KEYBOARD_LOWERCASE).size(); i++) {
            alphaLowerBitmapList
                    .add(createSpinnerItem(CharList.getInstance(gvrContext).getList(Keyboard.SOFT_KEYBOARD_LOWERCASE).get(i)));

        }

        for (int i = 0; i < CharList.getInstance(gvrContext).getList(Keyboard.SOFT_KEYBOARD_UPPERCASE).size(); i++) {
            alphaUpperBitmapList.add(createSpinnerItem(CharList.getInstance(gvrContext).getList(Keyboard.SOFT_KEYBOARD_UPPERCASE).get(i)
                    .toUpperCase()));
        }

        for (int i = 0; i < CharList.getInstance(gvrContext).getList(Keyboard.NUMERIC_KEYBOARD).size(); i++) {
            numericBitmapList.add(createSpinnerItem(CharList.getInstance(gvrContext).getList(Keyboard.NUMERIC_KEYBOARD).get(i)));
        }

        for (int i = 0; i < CharList.getInstance(gvrContext).getList(Keyboard.SOFT_KEYBOARD_SPECIAL).size(); i++) {
            specialBitmapList.add(createSpinnerItem(CharList.getInstance(gvrContext).getList(Keyboard.SOFT_KEYBOARD_SPECIAL).get(i)));
        }
