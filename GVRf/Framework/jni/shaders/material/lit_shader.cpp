...
static const char FRAGMENT_SHADER[] =
		"precision highp float;\n"
				...
				"  vec4 color = diffuse * materialDiffuseColor * lightDiffuseIntensity;\n"
				"\n"
...
LitShader::LitShader() :
		program_(0), a_position_(0), a_tex_coord_(0), a_normal_(0), u_mv_(0), u_mv_it_(
				0), u_mvp_(0), u_light_pos_(0), u_texture_(0), u_color_(0), u_opacity_(0),
				u_material_ambient_color_(0), u_material_diffuse_color_(0),
				u_material_specular_color_(0), u_material_specular_exponent_(0),
				u_light_ambient_intensity_(0), u_light_diffuse_intensity_(0), u_light_specular_intensity_(0){
	...
	u_light_pos_ = glGetUniformLocation(program_->id(), "u_light_pos");
	...
	u_material_ambient_color_ = glGetUniformLocation(program_->id(), "materialAmbientColor");
	u_material_diffuse_color_ = glGetUniformLocation(program_->id(), "materialDiffuseColor");
	u_material_specular_color_ = glGetUniformLocation(program_->id(), "materialSpecularColor");
	u_material_specular_exponent_ = glGetUniformLocation(program_->id(), "materialSpecularExponent");
	u_light_ambient_intensity_ = glGetUniformLocation(program_->id(), "lightAmbientIntensity");
	u_light_diffuse_intensity_ = glGetUniformLocation(program_->id(), "lightDiffuseIntensity");
	u_light_specular_intensity_ = glGetUniformLocation(program_->id(), "lightSpecularIntensity");
}
...
void LitShader::render(const glm::mat4& mv_matrix,
	...
	if (texture->getTarget() != GL_TEXTURE_2D) {
		std::string error = "LitShader::render : texture with wrong target.";
		throw error;
	}

	Light* light = render_data->light();
	if (!light) {
		std::string error = "LitShader::render : light is not set.";
		throw error;
	}
	...
	glm::vec4 light_specular_intensity = light->getVec4("specular_intensity");

#if _GVRF_USE_GLES3_
	mesh->setVertexLoc(a_position_);
	mesh->setTexCoordLoc(a_tex_coord_);
	mesh->setNormalLoc(a_normal_);
	mesh->generateVAO(Material::LIT_SHADER);
	...
	glUniform3f(u_light_pos_, light_position.x, light_position.y, light_position.z);
	glActiveTexture (GL_TEXTURE0);
	glBindTexture(texture->getTarget(), texture->getId());
	glUniform1i(u_texture_, 0);
	glUniform3f(u_color_, color.r, color.g, color.b);
	glUniform1f(u_opacity_, opacity);
	glUniform4f(u_material_ambient_color_, material_ambient_color.r, material_ambient_color.g, material_ambient_color.b, material_ambient_color.a);
	glUniform4f(u_material_diffuse_color_, material_diffuse_color.r, material_diffuse_color.g, material_diffuse_color.b, material_diffuse_color.a);
	glUniform4f(u_material_specular_color_, material_specular_color.r, material_specular_color.g, material_specular_color.b, material_specular_color.a);
	glUniform1f(u_material_specular_exponent_, material_specular_exponent);
	glUniform4f(u_light_ambient_intensity_, light_ambient_intensity.r, light_ambient_intensity.g, light_ambient_intensity.b, light_ambient_intensity.a);
	glUniform4f(u_light_diffuse_intensity_, light_diffuse_intensity.r, light_diffuse_intensity.g, light_diffuse_intensity.b, light_diffuse_intensity.a);
	glUniform4f(u_light_specular_intensity_, light_specular_intensity.r, light_specular_intensity.g, light_specular_intensity.b, light_specular_intensity.a);

	glBindVertexArray(mesh->getVAOId(Material::LIT_SHADER));
